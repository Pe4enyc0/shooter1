...
class Player(GameSprite):
    def update(self):
        ...
        elif keys[K_d] and self.rect.x < win_width - 85:  
            self.rect.x += self.speed
    def fire(self):
...
# FONT 
font.init()
text_font1 = font.SysFont("Comic Sans Ms", 36)
text_font2 = font.SysFont("Comic Sans Ms", 60)
...
def show_level(text_list, text_color): #створюємо фунцію
    ...
    for i in range(len(text_list)):
            mw.blit(text_font2.render(text_list[i], 1, text_color), (150, ((i+1)*100))) #рендеремо коженя рядок окремо
...
player = Player("rocket.png", 10, 250, win_height-100, 80, 100)
    ...
    if not finish:
        ...
        if rel_time == True:
            ...
            if now_time - last_time < 3:
                mw.blit(text_font1.render("Wait!!! reloading...", 1, (150,0,0)), (220,330))
            else:
                num_fire = 0
                rel_time = False
